Notebook path in this repo: `experiments/cf_mf_umap/`.

In order to run the notebook, first create a conda environment with the yml file provided. From the `experiments/cf_mf_umap` directory (or pass the full path to `-f`), run:

```bash
cd experiments/cf_mf_umap
